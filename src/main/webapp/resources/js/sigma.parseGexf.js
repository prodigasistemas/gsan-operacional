...
sigma.publicPrototype.parseGexf = function(gexf) {
	...
	var nodesAttributes = []; // The list of attributes of the nodes of the
								// graph that we build in json
	var edgesAttributes = []; // The list of attributes of the edges of the
								// graph that we build in json
	var attributesNodes = gexf.getElementsByTagName('attributes'); 
	for (i = 0; i < attributesNodes.length; i++) {
		var attributesNode = attributesNodes[i]; // attributesNode is each
													// xml node 'attributes'
													// (plural)
		if (attributesNode.getAttribute('class') == 'node') {
			var attributeNodes = attributesNode
					.getElementsByTagName('attribute'); // The list of xml nodes
														// 'attribute' (no 's')
			for (j = 0; j < attributeNodes.length; j++) {
				var attributeNode = attributeNodes[j]; // Each xml node
														// 'attribute'

				var id = attributeNode.getAttribute('id'), title = attributeNode
						.getAttribute('title'), type = attributeNode
						.getAttribute('type');
			...
			var attributeNodes = attributesNode
					.getElementsByTagName('attribute'); // The list of xml nodes 'attribute' (no 's')
			for (j = 0; j < attributeNodes.length; j++) {
	...
	var nodesNodes = gexf.getElementsByTagName('nodes') // The list of xml nodes
														// 'nodes' (plural)

	for (i = 0; i < nodesNodes.length; i++) {
		var nodesNode = nodesNodes[i]; // Each xml node 'nodes' (plural)
		var nodeNodes = nodesNode.getElementsByTagName('node'); // The list of
																// xml nodes
																// 'node' (no
																// 's')
		...
		for (j = 0; j < nodeNodes.length; j++) {
			...
			sizeNodes = sizeNodes.length || poorBrowserXmlNsSupport ? sizeNodes
			...
			positionNodes = positionNodes.length || poorBrowserXmlNsSupport ? positionNodes
			...
			colorNodes = colorNodes.length || poorBrowserXmlNsSupport ? colorNodes
					: nodeNode.getElementsByTagNameNS('*', 'color');
			if (colorNodes.length > 0) {
				colorNode = colorNodes[0];
				color = '#'
						+ sigma.tools.rgbToHex(parseFloat(colorNode
								.getAttribute('r')), parseFloat(colorNode
								.getAttribute('g')), parseFloat(colorNode
								.getAttribute('b')));
			}

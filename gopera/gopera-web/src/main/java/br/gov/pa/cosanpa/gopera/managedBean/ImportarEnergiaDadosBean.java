...
public class ImportarEnergiaDadosBean extends BaseBean<EnergiaEletricaDados>{
	...
	@EJB
	private IEnergiaEletricaDados fachada;
	@EJB
	private IEnergiaEletrica fachadaEnergia;	
	...
	private void iniciarLazy(){
		if (listaDados == null) {  
			listaDados = new LazyDataModel<EnergiaEletricaDados>() {
				...
				@Override
				public List<EnergiaEletricaDados> load(int startingAt, int maxPerPage, String sortField, SortOrder sortOrder, Map<String, String> filters) {
					try {					
	...
	public void excluirDados() {
		try {
			...
			mostrarMensagemErro("Erro ao Excluir Registro");
			e.printStackTrace();
		}
	}	

...
public class RsoBean extends BaseBean<RSO> {
	...
	private boolean validaMedidor(){
		Integer rso_id = this.registro.getCodigo();
		String verifica;

		try{
			if(rso_id != null) {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(this.registro.getMedidorEntrada().getCodigo(), rso_id, 4);
			} else {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(this.registro.getMedidorEntrada().getCodigo(), 0, 0);
			}
	...
	public List<EEAT> getListaEEAT() {
		try {
			listaEEAT = fachadaEEAT.obterLista(0, 0);
		} catch (Exception e) {

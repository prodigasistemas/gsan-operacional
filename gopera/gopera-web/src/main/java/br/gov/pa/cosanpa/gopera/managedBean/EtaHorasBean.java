...
import br.gov.pa.cosanpa.gopera.model.ETAHorasCMB;
...
@ManagedBean
@SessionScoped
public class EtaHorasBean extends BaseBean<ETAHoras> {

	private static Logger logger = Logger.getLogger(EtaHorasBean.class); 

	@EJB
	private IETAHoras fachada;
	@EJB
	...
	private LazyDataModel<ETAHoras> listaConsumo = null;	
	 ...
	 * PRIVATE METHODS
	 *********************************************/
	private Map<Integer, ETAHorasCMB> todasCMBsDaUnidade() throws Exception {
		Map<Integer, ETAHorasCMB> lista = new HashMap<Integer, ETAHorasCMB>();
		ETA eeat = fachadaETA.obterETA(this.registro.getEta().getCodigo());
		for (Integer cmbIndex = 1; cmbIndex <= eeat.getCmbQuantidade(); cmbIndex++) {
			ETAHorasCMB cmb = new ETAHorasCMB();
			cmb.setQuantidade(cmbIndex);
			cmb.setEtahoras(this.registro);
	...
	private void carregar(){
		try {
			this.registro = fachada.obterETAHoras(this.registro.getCodigo());
			DecimalFormat df = new DecimalFormat("#,##0.00");
			
			Map<Integer, ETAHorasCMB> todosCMBs = todasCMBsDaUnidade();
			for (ETAHorasCMB cmb : this.registro.getCmb()) {
				ETAHorasCMB horaCmb = todosCMBs.get(cmb.getQuantidade());
	...
	private void iniciarLazy(){
		if (listaConsumo == null) {  
			listaConsumo = new LazyDataModel<ETAHoras>() {
				private static final long serialVersionUID = 1L;

				@Override
				public List<ETAHoras> load(int startingAt, int maxPerPage, String sortField, SortOrder sortOrder, Map<String, String> filters) {
					try {					
						List<ETAHoras> listaLazy = fachada.obterListaLazy(startingAt, maxPerPage, filters);
				...
				@Override
				public Object getRowKey(ETAHoras consumo) {
					return consumo.getCodigo();
				}
				
				@Override
				public ETAHoras getRowData(String consumoId) {
					if (consumoId != null && !consumoId.equals("") && !consumoId.equals("null")) {
						Integer id = Integer.valueOf(consumoId);
						
						for (ETAHoras consumo : listaConsumo) {
	...
	public String iniciar() {
		...
		this.registro = new ETAHoras();
		// Páginas de mudança de estados
		this.getPaginasRetorno().put("iniciar", "EtaHoras.jsf");
	...
	public LazyDataModel<ETAHoras> getListaConsumo() {
		return listaConsumo;
	}

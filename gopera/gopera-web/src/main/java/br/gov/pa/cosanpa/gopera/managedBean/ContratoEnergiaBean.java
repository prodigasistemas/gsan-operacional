...
import br.gov.pa.cosanpa.gopera.model.ContratoEnergiaDemanda;
...
public class ContratoEnergiaBean extends BaseBean<ContratoEnergia> {
	...
	@EJB
	private IContratoEnergia fachada;
	@EJB
	...
	private void iniciarLazy(){
		if (listaContrato == null) {  
			listaContrato = new LazyDataModel<ContratoEnergia>() {
				...
				@Override
				public List<ContratoEnergia> load(int startingAt, int maxPerPage, String sortField, SortOrder sortOrder, Map<String, String> filters) {
					try {					
	...
	public List<UnidadeConsumidora> getListaUnidadeConsumidora() {
		try {
			listaUnidadeConsumidora = fachadaUC.obterLista(0, 0);
		} catch (Exception e) {

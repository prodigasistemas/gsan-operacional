...
public class EtaBean extends BaseBean<ETA> {
	...
	public List<EEAB> getListaEEAB() {
		try{
			listaEEAB = fachadaEEAB.obterLista(0, 0);
		} catch (Exception e) {
	...
	public void incluirFonte() {
		try{
			...
			Integer eta_id = this.registro.getCodigo();
			String verifica;
				
			if(eta_id != null) {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(ETAFonte.getMedidorEntrada().getCodigo(), eta_id, 6);
			} else {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(ETAFonte.getMedidorEntrada().getCodigo(), 0, 0);
			}

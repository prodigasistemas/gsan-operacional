...
public class RelatorioAnaliseLaboratorialBean extends BaseRelatorioBean<RelatorioGerencial> {
	...
	public void exibir() throws SQLException{
    	...
    	try{
			...
			nomeRelatorio = "Análises Laboratoriais";
    ...
    private void ExportaExcel(String nomeArquivo, ResultSet rs) throws Exception{
		...
		//Titulo			
		addLabel(sheet, 0, 0, "ANÁLISES LABORATORIAIS", wcfLabelBold);
		...
		addLabel(sheet, 5, linha, "AMOSTRA ANALISADA", wcfLabelHeader);
		addLabel(sheet, 6, linha, "AMOSTRA EM CONFORMIDADE", wcfLabelHeader);
		linha++;
		while (rs.next()) {
			//TOTALIZADORES	
			if (loca_qb){
			...
			addLabel(sheet, 4, linha, formataDataPadrao.format(formataDataSQL.parse(rs.getString("anac_referencia"))) , wcfLabel);
			valor1 = Double.parseDouble(rs.getString("anac_analisada"));
			valor2 = Double.parseDouble(rs.getString("anac_conformidade"));

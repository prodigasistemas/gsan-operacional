...
@ManagedBean
@SessionScoped
public class RelatorioRedeInstaladaBean extends BaseRelatorioBean<RelatorioGerencial> {
	...
	public String iniciar(){
		...
		return "RelatorioRedeInstalada.jsf";
	}	
	...
	public void exibir() throws SQLException{
    	...
    	try{
			...
			nomeRelatorio = "Adutoras e Redes de Água";
			...
			sql = "SELECT A.greg_id, greg_nmregional, A.uneg_id, uneg_nmunidadenegocio, A.muni_id, muni_nmmunicipio, A.loca_id, loca_nmlocalidade,"
			    + "		  rdin_referencia, rdin_existente, rdin_cadastrada"
			    ...
			    + " WHERE rdin_referencia BETWEEN '" + dataIni + "' AND '" + dataFim + "'" + sqlFiltro
			    + " ORDER BY greg_nmregional, uneg_nmunidadenegocio, muni_nmmunicipio, loca_nmlocalidade, rdin_referencia";
	...
	private void ExportaExcel(String nomeArquivo, ResultSet rs) throws Exception{
		...
		//Titulo			
		addLabel(sheet, 0, 0, "ADUTORAS E REDES DE ÁGUA", wcfLabelBold);
		sheet.mergeCells(0, 0, 6, 0);
		//Filtro
		addLabel(sheet, 0, 1, filtroRelatorio, wcfLabelLeft);
		sheet.mergeCells(0, 1, 6, 1);

		addLabel(sheet, 0, 2, "Referência: " + this.referenciaInicial + " a " + this.referenciaFinal, wcfLabelBold);
		...
		addLabel(sheet, 5, linha, "REDE EXISTENTE", wcfLabelHeader);
		addLabel(sheet, 6, linha, "REDE CADASTRADA", wcfLabelHeader);
		linha++;
		while (rs.next()) {
			//TOTALIZADORES	
			if (loca_qb){
				...
				addNumero(sheet, 5, linha, loca_total1, wcfInteiroBold);
				addNumero(sheet, 6, linha, loca_total2, wcfInteiroBold);
				loca_total1 = 0.0;
				loca_total2 = 0.0;
				linha++;
			}
			if (muni_qb){
				...
				addNumero(sheet, 5, linha, muni_total1, wcfInteiroBold);
				addNumero(sheet, 6, linha, muni_total2, wcfInteiroBold);
				muni_total1 = 0.0;
				muni_total2 = 0.0;
				linha++;
			}
			if (uneg_qb){
				...
				addNumero(sheet, 5, linha, uneg_total1, wcfInteiroBold);
				addNumero(sheet, 6, linha, uneg_total2, wcfInteiroBold);
				uneg_total1 = 0.0;
				uneg_total2 = 0.0;
				linha++;
			}			
			if (greg_qb){
				...
				addNumero(sheet, 5, linha, greg_total1, wcfInteiroBold);
				addNumero(sheet, 6, linha, greg_total2, wcfInteiroBold);
				greg_total1 = 0.0;
				greg_total2 = 0.0;
				linha++;
			}
			...
			addLabel(sheet, 4, linha, formataDataPadrao.format(formataDataSQL.parse(rs.getString("rdin_referencia"))) , wcfLabel);
			valor1 = Double.parseDouble(rs.getString("rdin_existente"));
			valor2 = Double.parseDouble(rs.getString("rdin_cadastrada"));
			addNumero(sheet, 5, linha, valor1, wcfInteiro);
			addNumero(sheet, 6, linha, valor2, wcfInteiro);
		...
		addNumero(sheet, 5, linha, total1, wcfInteiroBold);
		addNumero(sheet, 6, linha, total2, wcfInteiroBold);

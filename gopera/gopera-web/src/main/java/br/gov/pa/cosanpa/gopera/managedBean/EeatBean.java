...
public class EeatBean extends BaseBean<EEAT> {
	...
	public void incluirFonte() {
		try {
			...
			Integer eeat_id = this.registro.getCodigo();
			String verifica;
			
			if(eeat_id != null) {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(EEATFonte.getMedidorEntrada().getCodigo(), eeat_id, 7);
			} else {
				verifica = fachadaProxy.getVerificaMacroMedidorCadastrado(EEATFonte.getMedidorEntrada().getCodigo(), 0, 0);
			}
